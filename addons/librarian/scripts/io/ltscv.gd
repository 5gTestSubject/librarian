...
static func get_table_reader():
    return preload("res://addons/librarian/scripts/io/ltcsv_reader.gd").new()

static func get_table_writer():
    return preload("res://addons/librarian/scripts/io/ltcsv_writer.gd").new()
...
static func create_table(table_path: String, name: String) -> bool:
    ...
    metadata.name = name
    var writer = get_table_writer()
    writer.open(file_path, metadata)
    writer.flush()
    writer.close()
    return true
